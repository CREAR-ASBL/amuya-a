...
class LupascoPanelWritter {
    ...
    */
    LupascoPanelWritter(ArrayList<Disjunction> disjunctions, ArrayList<Integer> parents){
        settings();
        ...
        if ((LupascoPanel.SIGNS && LupascoPanel.FULL) || (LupascoPanel.SIGNS && !LupascoPanel.FULL)) {
            if (withSign){
                return new JLabel("<html>" + oConBegSpan + "(" + f1Abb + "&middot;" +
                        f2Abb +")&nbsp;" + CSPAN + ImplicationPanel.oI + "&sup;" + ImplicationPanel.CI + "</html>");
            } else if (!withSign){
    ...
    */
    ...
    JLabel getSFLabel(int subCon) {
        ...
        if (parents.isEmpty()){
            ...
            implication = new ImplicationPanel(disjunctions.get(0), parents.get(0));
    ...
    private JLabel getSALabel(int subCon) {
        ...
        // Same as get SFLabel() but with some modifications (no iteration)
            implication = new ImplicationPanel(
